...
- `AgentID-Setup.http` は REST Client の `{{$aadV2Token appOnly}}` を使用するため、[リポジトリルートの README.md](../README.md) の「Microsoft Entra ID にアプリ登録を行います」「環境変数を生成」>「Microsoft Graph API 向けの設定」および「ダウンロードしたサンプルを使用する」を完了しておくこと
